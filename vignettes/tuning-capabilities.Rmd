...
`metric_set()` is not limited to built-in metrics: it also accepts custom
metrics created with `yardstick::new_numeric_metric()` (see
`vignette("custom-metrics", package = "yardstick")`), so kindling models can
be tuned and assessed against user-defined criteria as well.
